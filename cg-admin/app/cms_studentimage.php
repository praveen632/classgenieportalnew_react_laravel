...
use DB;
use Session;
class cms_studentimage extends Model
{
	protected $table = ED_IMAGEMGMT;
	public function getdata(){
		return $imagemang = DB::table(ED_IMAGEMGMT)->where('type',2)->get();
	}
	public function adddata($request){
		$target_dir = base_path().'/public/assets/student/';
	...
	if ($uploadOk == 0) {
		...
		$insert = DB::table(ED_IMAGEMGMT)->insert(array('title'=>$request->title, 'order'=>$request->order, 'type'=>'2','created_at'=> date('Y-m-d H:i:s'),'updated_at'=> date('Y-m-d H:i:s')));
		if($insert){
...
public function deletedata($id){
	...
	unlink(base_path().'/public/assets/student/'.$image_name);
